...
import { Send, Bot, User, Sparkles, Zap, Brain, Target, TrendingUp, MessageSquare } from "lucide-react"
...
export default function AgentPage() {
  ...
  const [previousMentor, setPreviousMentor] = useState(mentors[0])
  ...
  const handleMentorChange = (mentor: typeof mentors[0]) => {
    setPreviousMentor(selectedMentor)
    setSelectedMentor(mentor)
    setShowMentors(false)
    // Reset messages when changing mentors
    setMessages([])
  }
          ...
          <div className="space-y-4">
            ...
            <div className="grid lg:grid-cols-12 gap-4 lg:gap-6">
              ...
              <div className="lg:col-span-9">
                <Card className={`flex flex-col border-2 bg-card shadow-xl rounded-3xl overflow-hidden h-[calc(100vh-280px)] sm:h-[calc(100vh-240px)] lg:h-[700px] transition-all duration-300 ${
                  selectedMentor.accentBorder
                }`}>
                  {/* Chat Header */}
                  <div className={`p-4 sm:p-6 border-b-2 bg-gradient-to-r ${selectedMentor.color} bg-opacity-10 transition-all duration-300`}>
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl ${selectedMentor.accentBg} flex items-center justify-center shadow-lg text-2xl`}>
                        {selectedMentor.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 bg-gradient-to-b from-transparent to-muted/20">
                    {isEmpty ? (
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-center py-12 px-4 space-y-6">
                        <div className={`w-16 h-16 rounded-2xl ${selectedMentor.accentBg} flex items-center justify-center text-4xl`}>
                          {selectedMentor.icon}
                        </div>
                        <div>
                        ...
                        <div className="w-full max-w-md space-y-2 pt-4">
                          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Suggested Questions</p>
                          <div className="space-y-2">
                            {suggestedPrompts[selectedMentor.name as keyof typeof suggestedPrompts]?.map((prompt, idx) => (
                              <button
                                key={idx}
                      ...
                      <>
                        {messages.map((message, index) => (
                          ...
                          >
                            {message.role === "assistant" && (
                              <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl ${selectedMentor.accentBg} flex items-center justify-center flex-shrink-0 border ${selectedMentor.accentBorder} text-lg`}>
                                {selectedMentor.icon}
                              </div>
                            )}
                            <div
                        ...
                        {isLoading && (
                          <div className="flex gap-2 sm:gap-3 animate-slide-up">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl ${selectedMentor.accentBg} flex items-center justify-center flex-shrink-0 border ${selectedMentor.accentBorder}`}>
                              {selectedMentor.icon}
                            </div>
